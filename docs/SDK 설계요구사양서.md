...
Greengrass 등 엣지 Python 환경에서 MQTT 5.0으로 클라이언트와 비동기 RPC를 수행할 때, 연결·구독·디스패치·응답 발행을 캡슐화한다. 개발자는 `@server.action("이름")`으로 페이로드 필드 `"action"`과 일치하는 핸들러만 등록하면 된다.

### 1.2 타겟 환경

- **언어:** Python 3.8+
- **전송:** AWS IoT Core MQTT (TLS 또는 WSS; 인증서·엔드포인트는 배포 환경에 맞게 구성)
...
- 선택: `$aws/events/presence/disconnected/+` 등 프레즌스 모니터링.

#### RPC 디스패처

- 수신 메시지의 **서비스 토픽**이 자신의 ThingType·Service·VIN과 일치하는지 검증한다.
- JSON 페이로드를 파싱하고 **`action`** 값으로 등록된 핸들러를 선택한다. (구 설계의 페이로드 `group` 등은 사용하지 않는다.)
...
- **`MaasServer(thing_type, service_name, vin, endpoint, ...)`** 로 인스턴스를 생성한다.
- **`@server.action(action_name, streaming=..., exclusive=..., acquire_lock=..., release_lock=...)`** 로 핸들러를 등록한다. `action_name`은 요청 JSON의 `"action"`과 동일해야 한다.
...
server = MaasServer(
    ...
    service_name="viss",
    vin="VIN-123456",
    endpoint="xxxx.iot.region.amazonaws.com",
)
...
브라우저·백오피스 등에서 AWS IoT Core(WSS)로 엣지 서비스를 호출할 때, 토픽 조립·`Correlation Data`·타임아웃·스트림 수신을 캡슐화한다. 개발자는 **`thing_type`, `service`, `action`, `vin`** 만 지정하면 된다.
...
- **`call(thing_type, service, action, vin, payload=..., qos=..., timeout=..., expiry=...)`**  
...
| [RPC 보안정책.md](RPC%20보안정책.md) | AGT/AT, Custom Authorizer, 동적 IoT 정책 |
