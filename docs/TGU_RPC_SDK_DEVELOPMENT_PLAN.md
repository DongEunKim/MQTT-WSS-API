...
### 2.7. MQTT RPC 방법론 (참조: `docs/MQTT_RPC_METHODOLOGY.md`, `docs/RPC_TRANSPORT_LAYER_DESIGN.md`)

**전송 계층 래퍼 관점**: MQTT/WSS payload는 RPC를 위한 중간 전송 계층이다. TGU는 payload에서 **응답 위치(response_topic)**를 명시적으로 읽어 처리한다. VISSv2 MQTT 패턴을 따른다.

| 항목 | 내용 |
|------|------|
| **토픽 분리** | Request: `WMT/{service}/{vehicle_id}/request` / Response: 클라이언트가 `response_topic`으로 명시 |
| **상관관계** | payload의 `request_id`로 요청-응답 매칭 |
| **클라이언트** | ① 응답 토픽 구독 ② 요청 발행 (response_topic 포함) ③ request_id 일치하는 응답 대기 ④ 타임아웃 처리 |
| **TGU** | ① 요청 토픽 구독 ② payload에서 response_topic, request 추출 ③ response_topic에 응답 발행 (토픽 패턴 불필요) |
...
- [x] 토픽 패턴 확정: `WMT/{service}/{vehicle_id}/request`, `WMO/{service}/{vehicle_id}/{client_id}/response`
- [x] Payload 상관관계: request_id, service, client_id 등 (RPC 방법론 문서 참조)
- [x] wss-mqtt-api ACL: WMT 발행 시 vehicle_id 접근 허가 검사, 미허가 시 거부
...
- `docs/MQTT_RPC_METHODOLOGY.md` — MQTT 기반 RPC 방법론 (토픽, payload, 시퀀스)
- `docs/RPC_TRANSPORT_LAYER_DESIGN.md` — RPC 전송 계층 설계 (VISSv2 참조, TGU 관점, response_topic)
...
| 항목 | 설명 | 예시 |
|------|------|------|
| **요청 토픽 패턴** | 클라이언트가 PUBLISH하는 토픽 형식 | `WMT/{service}/{vehicle_id}/request` |
| **응답 토픽 패턴** | RPC 응답 수신용 SUBSCRIBE 토픽 (client_id별 분리) | `WMO/{service}/{vehicle_id}/{client_id}/response` |
| **구독형 데이터 토픽** | VISSv3 스타일 이벤트/주기 발행 토픽 | (추가 확정 시) |
| **vehicle_id 대체** | 차량 식별자 외 다른 식별자 사용 여부 (예: device_id, session_id) | - |

### 8.1.1. RPC Payload 스키마 (필수)

> 상세: `docs/MQTT_RPC_METHODOLOGY.md`, `docs/RPC_TRANSPORT_LAYER_DESIGN.md`
...
| 0.7 | 2025-03-13 | - | VISSv2 MQTT 패턴 반영: response_topic 명시, request 중첩, RPC_TRANSPORT_LAYER_DESIGN 추가. call(service, payload) |
