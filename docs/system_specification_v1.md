...
| **TTL**                           | Time-to-Live. [옵션] 활동 기반 TTL 적용 시, 메시지 미수신 구독의 최대 유효 시간. 기본 정책(8.3)에서는 연결 기반으로 구독 유지.                                                                                  |
...
- **유지 대상:** 라우팅을 위한 Subscription Map (8.4 활동 기반 TTL 적용 시 타이머)
- **미유지 대상:** 사용자 식별자, 히스토리, 선호도 등 비라우팅 데이터
- **영속성 책임:** 데이터의 영속성은 MQTT 브로커 및 백엔드 서비스에 위임한다.

### 3.3. 명시적 권한 통제 (Explicit ACL)

모든 발행 및 구독 요청은 토픽 경로에 기반하여 인가(Authorization) 검사를 거친다. 토픽 패턴 및 필터 규칙은 [TOPIC_AND_ACL_SPEC.md](TOPIC_AND_ACL_SPEC.md)를 참고한다. 거절 시 클라이언트는 `403`(권한 없음) 또는 `422`(허용되지 않는 토픽 패턴)로 통보받는다.
...
- **재연결:** 클라이언트는 연결 끊김 시 exponential backoff로 재연결을 시도할 수 있다. 재연결 시 구독 상태는 8.5에 따라 초기화되므로, 활성 구독을 다시 SUBSCRIBE로 등록하는 것이 권장된다.
...
아래는 클라이언트 요청 및 게이트웨이 응답의 JSON 예시이다. 직렬화는 문자열(텍스트) WebSocket 프레임으로 전송한다.

**PUBLISH 요청 (클라이언트 → 게이트웨이):**
...
### 8.3. 구독 생명주기 (연결 기반, API 게이트웨이 측)

- **기본 정책:** 구독은 **WebSocket 연결이 유지되는 동안** 유효하다. 별도 TTL 없음.
- **동작:** `SUBSCRIBE` 수신 시 Subscription Map에 등록. 연결이 끊어질 때까지 유지.
- **목적:** RPC 응답용 단기 구독뿐 아니라, 스트리밍·모니터링 등 **장기 구독**도 지원.

### 8.4. [옵션] 활동 기반 TTL (구현자 선택)

WSS API 서버 구현 시, 리소스 회수 목적으로 다음을 **선택적으로** 적용할 수 있다.

- **동작:** 구독 수신 시 초기 TTL(예: 40~60초) 설정. 해당 구독으로 **메시지가 수신될 때마다** TTL을 갱신.
- **만료:** TTL 동안 메시지 수신이 없으면 구독 해제.
- **효과:** 활동 없는 구독만 정리. 스트리밍 등 지속 수신 중인 구독은 유지됨.
...
5. (8.4 옵션 적용 시) 활동 기반 TTL 타이머 가동
6. ACK(200) 전송
...
1. Subscription Map에서 해당 `topic`을 구독한 모든 `(session, req_id)` 조회 (8.4 적용 시 각 구독에 대해 TTL 갱신)
...
**TTL 만료 (8.4 옵션):** 활동 기반 TTL을 적용한 경우, 메시지 미수신 구독이 TTL 만료 시 제거된다.
