...
| # | 항목 | 우선순위 | 설명 |
|---|------|----------|------|
| 1 | WssMqttClientSync | 높음 | sync 래퍼 클래스 (asyncio.run 내부 활용) |
...
### 4.1 WssMqttClientSync

sync 래퍼. 내부에서 WssMqttClient + asyncio.run 사용.

```python
client = WssMqttClientSync(url=URL)
client.connect()
...
```python
with WssMqttClientSync(url=URL) as client:
    client.publish("topic", {"x": 1})
```
...
- `WssMqttClient` (async)는 그대로 유지
- 고급 사용자, 스트리밍, 다중 구독 등은 async API 사용
- `WssMqttClientSync`는 단순 사용 사례 전용 (발행, listen-only 구독)
...
| 순서 | 항목 | 이유 |
|------|------|------|
| 1 | WssMqttClientSync (connect/disconnect/publish) | 기반 클래스 |
...
| 파일 | 내용 |
|------|------|
| `client_sync.py` | WssMqttClientSync 클래스 |
| `__init__.py` | WssMqttClientSync 노출 |
| `examples/subscriber_sync.py` | sync 콜백 구독 예제 |
| `examples/publisher_sync.py` | sync 발행 예제 (선택) |
