...
본 시스템의 RPC는 MQTT 5.0 브로커를 통해 클라이언트와 서비스 간 통신을 구현한다.  
...
- **서버 라우팅 키:** `maas-server-sdk`는 기본으로 위 필드명을 `route_key="action"`으로 해석한다. 다른 키를 쓰려면 `MaasServer(..., route_key="...")`로 맞추고, 페이로드에서 라우팅 필드를 분리하지 않으려면 `route_key=None`과 `@server.default`를 사용한다([TOPIC_AND_ACL_SPEC.md](TOPIC_AND_ACL_SPEC.md) §5 참고).
...
- 인증: JWT 토큰을 MQTT username 등으로 전달 (브로커·게이트웨이 구성에 따름)
- Keep-Alive: 30초 (프록시·로드밸런서 idle timeout 대응)
- Clean Start: 항상 True (stale 응답 방지)
- **SDK 사용 패턴:** `maas-client-sdk`에서는 생성자에 `thing_type` / `service` / `vin`을 고정하고 `call(action[, params])`·`stream(action[, params])`·무인자 `exclusive_session()`을 권장한다. 플릿 등 호출마다 VIN이 바뀌면 `call(thing_type, service, action, vin[, params])`·`stream(...)`·`exclusive_session(thing_type, service, vin)`으로 네 축을 명시한다. `connect()` 할 때마다 `token_provider`가 호출되어 최신 토큰을 반영할 수 있다.

### 5.2 서비스 (MQTT 5.0 over TLS)

- 전송: TCP + TLS (포트 8883) 또는 WSS
- 인증: X.509 인증서 등(배포 환경에서 관리)
- Keep-Alive: 60초
...
서비스의 클라이언트 단절 감지:
- 브로커가 연결 수명 이벤트 토픽을 제공하면 `MaasServer(..., lifecycle_topics=[...])`로 구독
