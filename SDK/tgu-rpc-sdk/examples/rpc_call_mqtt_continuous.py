#!/usr/bin/env python3
"""
TGU RPC 연속 호출 예제 (구조적 지연 분석용).

단일 연결로 여러 RPC를 연속 호출하여, 호출별 소요 시간을 측정한다.
연결 재사용 vs 매번 새 연결 패턴을 비교할 수 있다.

Usage:
    # 연결 재사용 (권장 — 지연 패턴 확인)
    python SDK/tgu-rpc-sdk/examples/rpc_call_mqtt_continuous.py --count 20

    # 매 호출마다 새 연결 (rpc_call_mqtt 연속 실행과 동일)
    python SDK/tgu-rpc-sdk/examples/rpc_call_mqtt_continuous.py --count 20 --reconnect
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="TGU RPC 연속 호출 — 구조적 지연 분석"
    )
    parser.add_argument(
        "-n",
        "--count",
